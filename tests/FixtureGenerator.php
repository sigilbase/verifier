...
final class FixtureGenerator
{
    ...
    /**
     * A fresh random key and stream by default. A seed (32 bytes, hex) and a
     * stream id make the output deterministic, which is what the committed
     * consistency fixtures in corpus/ need: they are generated by
     * tools/build-corpus-consistency.php under the published corpus test key
     * and must come out byte for byte the same on every machine.
     */
    public function __construct(?string $seedHex = null, ?string $streamId = null)
    {
        require_once __DIR__.'/../verify.php';

        $keypair = $seedHex === null
            ? sodium_crypto_sign_keypair()
            : sodium_crypto_sign_seed_keypair((string) hex2bin($seedHex));
    ...
    public function zip(string $dir, string $zipPath): string
    {
        ...
        foreach (['manifest.json', 'events.ndjson', 'checkpoints.json'] as $file) {
            ...
            // A fixed entry time keeps a committed fixture byte-stable.
